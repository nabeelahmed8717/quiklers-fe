...
const NavBar = ({ activeRole }: any) => {
            ...
            label: (
                <div className="drp-items-nav-mul">
                    ...
                    <div className='switch-tabs'>
                        <div className="tabs-bx-s ss-t" onClick={() => { setIsSwitchModal(true); setContentsSwitchModal({ type: "serviceSeeker", role: "" }) }}>Service Seeker</div>
                        <div className="tabs-bx-s sp-t" onClick={() => { setIsSwitchModal(true); setContentsSwitchModal({ type: "serviceProvider", role: "" }) }}>Service Provider</div>
                        <div className="tabs-bx-s sc-t" onClick={() => { setIsSwitchModal(true); setContentsSwitchModal({ type: "collaborator", role: "" }) }}>Collaborator</div>
                    </div>
                </div>
            ),
        },
    ...
    return (
        ...
        >
            {!isMobile && <div className="brand-logo"><img src={matchingRoute ? brandLogoW : brandLogoB} alt="" /></div>}
            {
                ...
                <>
                    {!isMobile && <div className="advance-search">
                        <Input type='search' className='input-adv-search' placeholder='Search in Quiklers...' onClick={() => setIsAdvanceSearch(true)} suffix={<img src={searchIcon} alt="" width={20} height={20} />} />
                    </div>}
                </>
            }
                        ...
                        <>
                            {
                                serviceProviderRoutesArray.map((item: any) => (
                                    <div className={`item-bx ${item.route === route ? 'ite-selected' : ''}`} onClick={() => navigate('.' + item.route)}>
                                        <p>{item.label}</p>
                                        {
                                            item.route === route &&
                                            <div className="nvg"></div>
                                        }
                                    </div>
                                ))
                            }
                        ...
                        <>
                            {
                                ...
                                <>
                                    ...
                                    <Button className="rounded-buttons-nav" onClick={() => navigate('./bookings')}><img src={bookings} width={20} height={20} alt="" /></Button>
                                </>
                            }
                            <Button className="rounded-buttons-nav" onClick={() => navigate('./chat')}><img src={chat} width={20} height={20} alt="" /></Button>
                            <Button className="rounded-buttons-nav" onClick={() => navigate('./chat')}><img src={chat} width={20} height={20} alt="" /></Button>
            ...
            {isServiceSeeker && <AdvanceSearch isAdvanceSearch={isAdvanceSearch} setIsAdvanceSearch={setIsAdvanceSearch} />}

            <AvgModal isOpenModal={isSwitchModal} setIsOpenModal={setIsSwitchModal} contentsSwitchModal={contentsSwitchModal} setContentsSwitchModal={setContentsSwitchModal} isAvg onSubmit={() => alert('Clicked')} />
